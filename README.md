...
- \*`GITHUB_OAUTH_TOKEN`: access token for GitHub API (required - necessary for GitHub-hosted credential status/revocation management)
- `GITHUB_ORG`: name of GitHub organization (required - necessary for GitHub-hosted credential status/revocation management)
- \*`GITHUB_CRED_STATUS_REPO`: name of GitHub credential status repo (required - necessary for GitHub-hosted credential status/revocation management)
...
\*NOTE: Here are the steps for retrieving a valid value for `GITHUB_OAUTH_TOKEN` (Please reserve this step for after you have run `npm run setup` or `yarn setup` per the instructions above):
- Retrieve access token for GitHub API
- Login to GitHub as an authorized member of the organization
- Click on your profile dropdown icon in the top-right corner of the screen
- Select the *Settings* tab
- Select the *Developer settings* tab toward the bottom of the left navigation bar
- Select the *Personal access tokens* tab
- Click *Generate a new token*
- Select the full *repo* scope
- Click *Generate token*
- Copy the generated token
- \*Save the token from the previous step as the `GITHUB_OAUTH_TOKEN` environment variable the service that is hosting your organization’s instance of *sign-and-verify*

\*NOTE: `GITHUB_CRED_STATUS_REPO`, the designated credential status repo, will automatically be generated after running `npm run start` or `yarn start`, per the instructions below. After this repo is autogenerated, here are the steps for enabling GitHub Pages hosting for credential status lists from this repo:
...
- Take note of the expiration date of `GITHUB_OAUTH_TOKEN` value generated above, so that you can renew this binding in advance. Failure to do this will result in a faulty credential status management process.
- Issuers should wait a couple of minutes after configuring GitHub Pages for credential status management before issuing credentials leveraging the hosted status list. This is because there is a delay between configuration and publication.
