...
\*NOTE: Here are the steps for retrieving a valid value for `GITHUB_OAUTH_TOKEN` (Please reserve this step for after you have run `npm run setup` or `yarn setup` per the instructions below):
...
11. \*Save the token from the previous step as the `GITHUB_OAUTH_TOKEN` environment variable the service that is hosting your organization’s instance of *sign-and-verify*
