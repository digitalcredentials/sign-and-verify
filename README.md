...
- \*`GITHUB_API_ACCESS_TOKEN`: access token for GitHub API (required - necessary for GitHub-hosted credential status/revocation management)
...
- `CRED_STATUS_REPO_OWNER`: name of source code owner (required - necessary for delegated hosting of credential status/revocation management)
...
\*NOTE: Here are the steps for retrieving a valid value for `GITHUB_API_ACCESS_TOKEN` (Please reserve this step for after you have run `npm run setup` or `yarn setup` per the instructions below):
...
11. \*Save the token from the previous step as the `GITHUB_API_ACCESS_TOKEN` environment variable at the hosting service that is hosting your organization’s instance of *sign-and-verify*

\*NOTE: `CRED_STATUS_REPO_NAME`, the designated credential status repo, will automatically be generated after running `npm run start` or `yarn start`, per the instructions below. After this repo is autogenerated, here are the steps for enabling GitHub Pages hosting for credential status lists from this repo:
1. Select the *Settings* tab in the credential status repo
2. Select the *Pages* tab from the left navigation bar
3. Select the *main* branch in the **Source** section (keep the default *root* folder)
4. Click *Save*
...
- Take note of the expiration date of `GITHUB_API_ACCESS_TOKEN` value generated above, so that you can renew this binding in advance. Failure to do this will result in a faulty credential status management process.
