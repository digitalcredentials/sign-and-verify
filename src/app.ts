...
export async function build(opts = {}) {
  ...
  const { didSeed, demoIssuerMethod, credentialRequestHandler } = getConfig();
  ...
  server.post(
    '/request/credential', async (request, reply) => {
      ...
      // retrieved from issuer query parameter in first URL delivered
      // by issuer via diploma, email, LMS (e.g., Canvas), etc.
      const verificationMethod = requestInfo.verificationMethod;
